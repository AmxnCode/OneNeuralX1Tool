"""
Full Training Script - 26M Parameter Enhanced Model
Uses proper BPE tokenizer (not character-level).
"""
...
class MultiToolDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        query_ids = query_enc.ids
        
        # Tokenize answer (decoder target)
        answer_enc = self.tokenizer.encode(ex["answers"])
        answer_ids = answer_enc.ids
        
        # Pad/truncate
        query_ids = query_ids[:self.max_len] + [0] * max(0, self.max_len - len(query_ids))
        answer_ids = [1] + answer_ids[:self.max_len-1] + [0] * max(0, self.max_len - len(answer_ids) - 1)
        labels = answer_ids[1:] + [-100]
...
def train():
    print("=" * 60)
    print("Full Training - 26M Enhanced Model (BPE Tokenizer)")
    print("=" * 60)
    ...
    print(f"   Memory (ternary): {memory_mb:.2f} MB")
    ...
    for epoch in range(config["epochs"]):
        ...
        for i, batch in enumerate(train_loader):
            ...
            if "load_balance_loss" in out and out["load_balance_loss"] is not None:
    ...
    # Delete best checkpoint (we keep final)
    import glob, os
    best_path = "/Users/amanpreetsingh/projects/experiment/oneaimodel/checkpoints/x1_26m_best.pt"
    if os.path.exists(best_path) and os.path.exists(final_path) and best_path != final_path:
        os.remove(best_path)
        print(f"Deleted old best checkpoint")
    
    history_path = "/Users/amanpreetsingh/projects/experiment/oneaimodel/checkpoints/x1_26m_history.json"
